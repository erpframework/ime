			...
			switch ($subaction) {
				...
				case 'delete':
                                    ...
                                    if((int)$list > 0){
                                        $db = IEM::getDatabase();
                                        $job_to_check = false;
                                        $query = "SELECT COUNT(*) FROM [|PREFIX|]jobs_lists WHERE listid = ".(int)$list;	
                                        $result = $db->Query($query);
                                        $job_to_check = $db->FetchOne($result);
                                        $db->FreeResult($result);
                                        if((int)$job_to_check <= 0){$job_to_check = false;}                   
                                        if($job_to_check != false){
                                            $query = "SELECT jobstatus FROM [|PREFIX|]jobs WHERE jobid = ".(int)$list;	
                                            $result = $db->Query($query);
                                            if(!$result){
                                                trigger_error(mysql_error());
                                                FlashMessage('Unable to load list jobs. <br /> '. mysql_error(), SS_FLASH_MSG_ERROR, IEM::urlFor('Lists'));
                                            }
                                            $job_row = $db->FetchOne($result);
                                            if($job_row['jobstatus'] != 'c'){
                                                FlashMessage('Unable to delete contacts from list id '.(int)$list.'. Please cancel any jobs for that list in order to delete it.', SS_FLASH_MSG_ERROR, IEM::urlFor('Lists'));
                                            }
                                            $db->FreeResult($result);
                                        }
                                    } else {
                                        trigger_error(mysql_error());
                                        FlashMessage('List id did not post correctly. Please try again.', SS_FLASH_MSG_ERROR, IEM::urlFor('Lists'));
