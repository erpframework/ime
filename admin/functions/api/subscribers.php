...
class Subscribers_API extends API
{
	...
	function GetSubscriberIdsToConfirm($email='', $confirmcode='')
	{
		...
		$query = "SELECT subscriberid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers WHERE emailaddress='" . $this->Db->Quote($email) . "' AND confirmed = 0 AND confirmcode='" . $this->Db->Quote($confirmcode) . "'";
